...
mutate(total = log(rowSums(cbind(fare_amount, tip_amount))))
...
ggmap(manhattan, darken = 0.5) +
   ...
   geom_bin2d(data = taxi, aes(x=long, y=lat), bins=60, alpha=0.6) + labs(x='Latitude', y='Longitude', fill='Journeys')
...
ggmap(manhattan, darken = 0.5) +
   scale_fill_viridis(option = 'plasma') +
   stat_summary_2d(data = taxi, fun=mean, aes(x=long, y=lat, z=pred_total), bins=60, alpha=0.6) + labs(x='Latitude', y='Longitude', fill='Mean Predicted Fare')
...
ggmap(manhattan, darken = 0.5) +
   scale_fill_viridis(option = 'plasma') +
   stat_summary_2d(data = taxi, fun=mean_if_enough_data, aes(x=long, y=lat, z=total), bins=60, alpha=0.6) + labs(x='Latitude', y='Longitude', fill='Mean Predicted Fare')
